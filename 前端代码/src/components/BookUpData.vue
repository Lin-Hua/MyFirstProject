...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('http://localhost:8082/superadmin/modifybooks',_this.ruleForm).then(function (resp){
            _this.$alert('修改成功', '提示', {
  ...
  created () {
    const _this = this
    axios.get('http://localhost:8082/superadmin/getBookById/'+this.$route.query.bno).then(function (resp){
